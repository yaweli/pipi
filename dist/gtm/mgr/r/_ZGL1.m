...
GD	;
	...
	Q
READ	;
	N RING,X,C,E,CC,PO,US,MD,ZV S US="DEF",ZV="GTM",RING=0
	D noecho^%ZGL U 0:ESC W *27,"[?7l" U 0:WIDTH=0
...
GLLE	;
	...
	Q
SHORT	;
	I %?.N S %=$G(^%ZGL("SC",%)) D SHOW Q
	I %[">" S ^%ZGL("SC",$P(%,">",1))=$P(%,">",2,99) S %=""  W !,"SAVED .. ",! D SHOW
	Q
...
HELP	;
	...
	W !,"                         -- ^ERR(20130508,'17:25','SP',1) = 0_HO_123500_RQ_OK"
	W !,"EXAMPLE: "
	w !," 33>W(JB,AP,SC,LN        -- store new shorcut"
	w !," 33<tab>                 -- use shortcut"
	w !,""
	w !,"global(a,b,<tab>         -- return first $ORDER item"
	w !,"global(a,b,-1<tab>       -- return last $ORDER(,-1) item"
	w !,""
	w !,"global(a,b,::%D[""XYZ""    -- full dsm style conditinal display, any mumps condition after  :: , %D-data ,%G full global, %S single index"
	w !,""
	w !
	W !,"List of shortcuts:"
	N I S I="" F  S I=$O(^%ZGL("SC",I)) Q:I=""  W !,I," -- ",^(I)
	Q
UNDEF	;
	Q
	N US
